...
#define ID_SUGAR_125					120
#define ID_SUGAR_100					121
#define ID_SUGAR_75						122
#define ID_SUGAR_50						123
#define ID_SUGAR_25						124
#define ID_SUGAR_0						125
...
IMPLEMENT_SERIAL(CNormalContainerCtrl, CBCGPVisualContainerCtrl, 1)
//////////////////////////////////////////////////////////////////////
// Construction/Destruction
//////////////////////////////////////////////////////////////////////

CNormalContainerCtrl::CNormalContainerCtrl():
	m_pView(NULL)
{
...
BEGIN_MESSAGE_MAP(CNormalContainerCtrl, CBCGPVisualContainerCtrl)
	//{{AFX_MSG_MAP(CNormalContainerCtrl)
	ON_WM_CREATE()
	//}}AFX_MSG_MAP
END_MESSAGE_MAP()

int CNormalContainerCtrl::OnCreate(LPCREATESTRUCT lpCreateStruct)
{
...
void CNormalContainerCtrl::InitImage()
{
	//ICE IMAGE
...
void CNormalContainerCtrl::SetImage(UINT nID, CString szImagePath)
{
	CBCGPVisualContainer* pContainer = GetVisualContainer();
	ASSERT_VALID(pContainer);
...
void CNormalContainerCtrl::CleanAllTopping()
{
	CBCGPVisualContainer* pContainer = GetVisualContainer();
	ASSERT_VALID(pContainer);
...
void CNormalContainerCtrl::SelectedTopping()
{
	CBCGPVisualContainer* pContainer = GetVisualContainer();
	ASSERT_VALID(pContainer);
	for (int i = ID_TOPPING_PEARL; i <= ID_TOPPING_OREO; i++)
...
void CNormalContainerCtrl::SetImage(UINT nID, UINT nImageID)
{
	CBCGPVisualContainer* pContainer = GetVisualContainer();
	ASSERT_VALID(pContainer);
...
void CNormalContainerCtrl::CommandInit(const CString& szProduct)
{
	CBCGPVisualContainer* pContainer = GetVisualContainer();
	ASSERT_VALID(pContainer);
	InitImage();
	...
	if (!gCurProduct.szSugar.CompareNoCase(L"125%"))
	{
		CBCGPColorIndicatorImpl* pColor = DYNAMIC_DOWNCAST(CBCGPColorIndicatorImpl, pContainer->GetByID(ID_SUGAR_125));
		m_pLastColorIndicator = pColor;
		pColor->SetSelectColor(CBCGPBrush(CBCGPColor::GreenYellow), TRUE);
	}
	else if (!gCurProduct.szSugar.CompareNoCase(L"100%"))
	{
		CBCGPColorIndicatorImpl* pColor = DYNAMIC_DOWNCAST(CBCGPColorIndicatorImpl, pContainer->GetByID(ID_SUGAR_100));
		m_pLastColorIndicator = pColor;
		pColor->SetSelectColor(CBCGPBrush(CBCGPColor::GreenYellow), TRUE);
	}
	else if (!gCurProduct.szSugar.CompareNoCase(L"75%"))
	{
		CBCGPColorIndicatorImpl* pColor = DYNAMIC_DOWNCAST(CBCGPColorIndicatorImpl, pContainer->GetByID(ID_SUGAR_75));
		m_pLastColorIndicator = pColor;
		pColor->SetSelectColor(CBCGPBrush(CBCGPColor::GreenYellow), TRUE);
	}
	else if (!gCurProduct.szSugar.CompareNoCase(L"25%"))
	{
		CBCGPColorIndicatorImpl* pColor = DYNAMIC_DOWNCAST(CBCGPColorIndicatorImpl, pContainer->GetByID(ID_SUGAR_25));
		m_pLastColorIndicator = pColor;
		pColor->SetSelectColor(CBCGPBrush(CBCGPColor::GreenYellow), TRUE);
	}
	else if (!gCurProduct.szSugar.CompareNoCase(L"0%"))
	{
		CBCGPColorIndicatorImpl* pColor = DYNAMIC_DOWNCAST(CBCGPColorIndicatorImpl, pContainer->GetByID(ID_SUGAR_0));
		m_pLastColorIndicator = pColor;
		pColor->SetSelectColor(CBCGPBrush(CBCGPColor::GreenYellow), TRUE);
	}
	else if (!gCurProduct.szSugar.CompareNoCase(L"50%"))
	{
		CBCGPColorIndicatorImpl* pColor = DYNAMIC_DOWNCAST(CBCGPColorIndicatorImpl, pContainer->GetByID(ID_SUGAR_50));
		m_pLastColorIndicator = pColor;
		pColor->SetSelectColor(CBCGPBrush(CBCGPColor::GreenYellow), TRUE);
	}
	...
	m_szSugar = L"- " + gCurProduct.szSugar;
	SetTextInfo(ID_SUGAR_TEXT, m_szSugar);
...
void CNormalContainerCtrl::SetTextInfo(UINT nID,CString szText)
{
	CBCGPVisualContainer* pContainer = GetVisualContainer();
	ASSERT_VALID(pContainer);
...
BOOL CNormalContainerCtrl::OnMouseDown(int nButton, const CBCGPPoint& pt)
{
	USES_CONVERSION;
	CBCGPVisualContainer* pContainer = GetVisualContainer();
	ASSERT_VALID(pContainer);
	if (pContainer != NULL && !pContainer->IsSingleSel())
	{
		...
		if (pImage != NULL && pImage->GetID()>=ID_IMAGE_MORE_ICE && pImage->GetID()<= ID_IMAGE_ROOM_TEMP)
		{
			...
			/*switch (nID)
			{
			case ID_IMAGE_MORE_ICE:
				m_szIce = pImage->GetName();
				break;
			case ID_IMAGE_NORMAL_ICE:
				m_szIce = _T("NORMAL ICE");
				break;
			case ID_IMAGE_LESS_ICE:
				m_szIce = _T("LESS ICE");
				break;
			case ID_IMAGE_FILTER_ICE:
				m_szIce = _T("FILTER ICE");
				break;
			case ID_IMAGE_ROOM_TEMP:
				m_szIce = _T("ROOM TEMP");
				break;
			case ID_IMAGE_WARM_WATER:
				m_szIce = _T("WARM");
				break;
			}*/
		...
		if (pColor != NULL)
		{
			...
			{
			case ID_SUGAR_125:
				SetTextInfo(ID_SUGAR_TEXT, L"- 125%");
				m_szSugar = "125%";
				break;
			case ID_SUGAR_100:
				SetTextInfo(ID_SUGAR_TEXT, L"- 100%");
				m_szSugar = "100%";
				break;
			case ID_SUGAR_75:
				SetTextInfo(ID_SUGAR_TEXT, L"- 75%");
				m_szSugar = "75%";
				break;
			case ID_SUGAR_50:
				SetTextInfo(ID_SUGAR_TEXT, L"- 50%");
				m_szSugar = "50%";
				break;
			case ID_SUGAR_25:
				SetTextInfo(ID_SUGAR_TEXT, L"- 25%");
				m_szSugar = "25%";
				break;
			case ID_SUGAR_0:
				SetTextInfo(ID_SUGAR_TEXT, L"- 0%");
				m_szSugar = "0%";
				break;
			}
			gCurProduct.szSugar = m_szSugar;
		}
	}
	return FALSE;
}

void CNormalContainerCtrl::OnDraw(CBCGPGraphicsManager * pGM, const CBCGPRect & rectClip)
{
