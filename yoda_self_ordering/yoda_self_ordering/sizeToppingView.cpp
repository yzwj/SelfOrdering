...
CSizeToppingContainerCtrl::CSizeToppingContainerCtrl() :
	m_pView(NULL)
{
...
BEGIN_MESSAGE_MAP(CSizeToppingContainerCtrl, CBCGPVisualContainerCtrl)
	//{{AFX_MSG_MAP(CSizeToppingContainerCtrl)
	ON_WM_CREATE()
	//}}AFX_MSG_MAP
END_MESSAGE_MAP()

int CSizeToppingContainerCtrl::OnCreate(LPCREATESTRUCT lpCreateStruct)
{
...
void CSizeToppingContainerCtrl::InitImage()
{
...
void CSizeToppingContainerCtrl::SetImage(UINT nID, CString szImagePath)
{
	CBCGPVisualContainer* pContainer = GetVisualContainer();
	ASSERT_VALID(pContainer);
...
void CSizeToppingContainerCtrl::CleanAllTopping()
{
	CBCGPVisualContainer* pContainer = GetVisualContainer();
	ASSERT_VALID(pContainer);
...
void CSizeToppingContainerCtrl::SelectedTopping()
{
	CBCGPVisualContainer* pContainer = GetVisualContainer();
	ASSERT_VALID(pContainer);
	for (int i = ID_TOPPING_PEARL; i <= ID_TOPPING_OREO; i++)
...
void CSizeToppingContainerCtrl::SetImage(UINT nID, UINT nImageID)
{
	CBCGPVisualContainer* pContainer = GetVisualContainer();
	ASSERT_VALID(pContainer);
...
void CSizeToppingContainerCtrl::CommandInit(const CString& szProduct)
{
	CBCGPVisualContainer* pContainer = GetVisualContainer();
	ASSERT_VALID(pContainer);
	InitImage();
...
void CSizeToppingContainerCtrl::SetTextInfo(UINT nID, CString szText)
{
	CBCGPVisualContainer* pContainer = GetVisualContainer();
	ASSERT_VALID(pContainer);
...
BOOL CSizeToppingContainerCtrl::OnMouseDown(int nButton, const CBCGPPoint& pt)
{
	USES_CONVERSION;
	CBCGPVisualContainer* pContainer = GetVisualContainer();
	ASSERT_VALID(pContainer);
	if (pContainer != NULL && !pContainer->IsSingleSel())
	{
		...
		if (pImage != NULL && pImage->GetID() >= ID_SIZE_NORMAL && pImage->GetID() <= ID_SIZE_TALL_L)
		{
...
void CSizeToppingContainerCtrl::OnDraw(CBCGPGraphicsManager * pGM, const CBCGPRect & rectClip)
{
