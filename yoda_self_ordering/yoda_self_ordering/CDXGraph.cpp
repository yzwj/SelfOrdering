...
bool CDXGraph::QueryInterfaces(void)
{
	if (mGraph)
	{
		...
		hr |= mGraph->QueryInterface(IID_IMediaEventEx, (void **)&mEvent2);
...
IMediaEventEx * CDXGraph::GetEventHandle2(void)
{
	return mEvent2;
}
...
bool CDXGraph::SetNotifyWindow2(HWND inWindow)
{
	if (mEvent2)
	{
		mEvent2->SetNotifyWindow((OAHWND)inWindow, WM_GRAPHNOTIFY, 1);
		return true;
	}
	return false;
}
void CDXGraph::HandleEvent(WPARAM inWParam, LPARAM inLParam)
{
	...
	{
		LONG eventCode = 0, eventParam1 = 0, eventParam2 = 0;
		while (SUCCEEDED(mEvent2->GetEvent(&eventCode, &eventParam1, &eventParam2, 0)))
		{
			mEvent2->FreeEventParams(eventCode, eventParam1, eventParam2);
			switch (eventCode)
			{
			case EC_COMPLETE:
				AfxMessageBox(L"END");
...
double CDXGraph::GetPlaybackRate()
{
	double dbSpeed=0;
	if (mSeeking)
	{
		
		if (SUCCEEDED(mSeeking->GetRate(&dbSpeed)))
		{
			return dbSpeed;
		}
	}
...
bool CDXGraph::RenderFile(const char * inFile)
{
	if (mGraph)
	{
		...
		hr = mGraph->AddFilter(pVmr, L"Video Mixing Renderer");
...
BOOL CDXGraph::GetHeightWidth(int &nWIdth, int &nHeight)
{
	if (mBasicVideo)
	{
		long bitmapSize = 0;
